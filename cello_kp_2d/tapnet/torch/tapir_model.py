...
import re
if StrictVersion(re.search('\d+.\d+.\d+',torch.__version__).group()) < StrictVersion('2.1.0'):
    from tapnet.torch.nn import LayerNorm,Conv2d
else:
...
class TAPIR(nn.Module):
  ...
  def __init__(
    ...
    self.torch_cost_volume_track_mods = nn.ModuleDict({
        'hid1': Conv2d(1, 16, 3, 1, 1),
        'hid2': Conv2d(16, 1, 3, 1, 1),
        'hid3': Conv2d(16, 32, 3, 2, 0),
