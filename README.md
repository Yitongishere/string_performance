...
## 1. Install & Data Prepration

Clone this repo, and install the dependencies.

The total installation process includes three steps.

### 1.1 Create Virtual Environment and Install by Pip
...
### 1.3 MMCV installation (for human keypoints detection from 2D imagery)
...
### 1.4 CREPE installation (for pitch tracking)
...
### 1.5 TAP-Net installation (for instrument keypoints tracking from 2D imagery)
...
1.5.1 Install requirements for inference:
`pip install -r requirements_inference.txt`

1.5.2 Please first switch to the working directory for tapnet:
...
1.5.3 Download the checkpoint: (Optional, TrackKeypoints.py could automatically download it)
```
mkdir checkpoints
cd checkpoints
