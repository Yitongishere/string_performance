...
# TODO: Edit the violin LINKS
VIOLIN_LINKS = [[133, 134],
                [133, 135],
                [134, 136],
                [135, 137],
                [136, 137]]
...
# TODO： Edit the string links
STRING_LINKS = [[138, 139],
                [140, 141],
                [142, 143],
                [144, 145]]
...
def visualize_3d(data, proj_path, file_path='tri_3d', view_angle='whole'):
    ...
    for f in range(framenum):
        ...
        if zoom_in:
            ...
            # TODO edit keypoints num
            if key_points_num > 138:
                axes3.scatter(kp_3d[138:146, 0],
                              kp_3d[138:146, 1],
                              kp_3d[138:146, 2], c='black', s=5, zorder=98)
            if True not in np.isnan(kp_3d[146]):
                axes3.scatter(kp_3d[146, 0],
                              kp_3d[146, 1],
                              kp_3d[146, 2], c='r', s=30,
            ...
            violin_coll_3d = Line3DCollection(violin_segs3d, edgecolors='saddlebrown', linewidths=1, zorder=1)
            # bow_coll_3d = Line3DCollection(bow_segs3d, edgecolors='goldenrod', linewidths=1, zorder=1)

            axes3.add_collection(human_coll_3d)
            axes3.add_collection(violin_coll_3d)
...
if __name__ == "__main__":
    ...
    # TODO edit used cams
    used_cams = ['cam2', 'cam3', 'cam4', 'cam5', 'cam6', 'cam7']
    ...
    for ff in range(start_frame, end_frame + 1):
        ...
        for cc in used_cams:
            try:
                ...
                cam_ff.remove(cc)
                continue
            # TODO edit violin keypoint number
            violin_2d_cc_ff = np.zeros([5, 3])  # 9 violin key points in total (default score 0 will not be used)
            try:
                violin_json_path = f'../cello_kp_2d/kp_result/{proj_dir}/{CAM_DICT[cc]}/{ff}.json'
                violin_keypoints = json.load(open(violin_json_path))
                # Resolve XML
                ...
                #         violin_2d_cc_ff[kpt_idx] = [kpt_xy[0], kpt_xy[1], 1]  # set the human-label score to be 1
                for idx, violin_keypoint in enumerate(violin_keypoints):
                    # violin_2d_cc_ff[idx] = [violin_keypoint[0], violin_keypoint[1], violin_keypoint[2]]
                    # TODO should be set back to origin
                    violin_2d_cc_ff[idx] = [violin_keypoint[0], violin_keypoint[1], 1]
                kp_2d_cc_ff = np.concatenate([human_2d_cc_ff, violin_2d_cc_ff], axis=0)
        ...
        keypoint_num = kp_2d_all_cams.shape[1]
        kp_3d = ransac_triangulate_joints(kp_2d_all_cams, proj_mat, num_kpt=keypoint_num, epsilon=100, kpt_thr=0.55)
        if np.isnan(kp_3d).any():
            print(f'Frame {ff}: {kp_3d}')
            raise Exception('NAN occurs. Please adjust epsilon and key point threshold of ransac...')
        # kp_3d_all[ff] = kp_3d
