...
if __name__ == "__main__":
    ...
    plt.scatter(results[:, 0], results[:, 1], c=results[:, 2], s=1.5, cmap="OrRd")
    plt.colorbar()
    # plt.savefig("./pitch_detection.png")
    plt.show()
    ...
    for step in results:
        ...
        if step[2] >= conf_threshold:
            ...
            for i in range(len(PITCH_RANGES)):
                # 留50音分的误差包容度
                if (freq_cur > cent_dev(PITCH_RANGES[i][0], -50)
                    and freq_cur < cent_dev(PITCH_RANGES[i][1], 50)):
