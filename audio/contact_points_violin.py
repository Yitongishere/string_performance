...
import freq_position_violin
import librosa
from triangulation.smooth import Savgol_Filter
from triangulation.triangulation_violin import visualize_3d
...
def draw_contact_points(data, proj, file_name):
    ...
    for frame_idx, frame in enumerate(data):
        ...
        ax.set_title(f'Violin Strings (Frame {frame_idx + 1})')
...
def mapping(proj_dir, positions, visualize=False):
    ...
    for frame, kp_3d in enumerate(kp_3d_all):
        ...
        for pos_idx, ratio in enumerate(position):
            if ratio > 0:
                ...
                string_fund_freq = freq_position_violin.PITCH_RANGES[pos_idx][0]
                current_freq_list.append(freq_position_violin.positon2freq(string_fund_freq, ratio))
        ...
        if not np.isnan(current_freq):
            within_last_range = freq_position_violin.cent_dev(last_freq, -30) < current_freq < freq_position_violin.cent_dev(last_freq, 30)
            if within_last_range:
                ...
                within_mean_range = freq_position_violin.cent_dev(vibrato_freq_mean, -30) < current_freq < freq_position_violin.cent_dev(vibrato_freq_mean, 30)
        ...
        # TODO index range modify
        # index from 138 to 150
        temp_list.extend(
    ...
    # Bow Points are currently not available, otherwise index should be set to 140
    # Bow Points should be adjusted
    kp_3d_all = kp_3d_all_cp[:, :138, :]
    kp_3d_all_smooth = Savgol_Filter(kp_3d_all, 138)
    # ic(kp_3d_all_smooth.shape)
    cp = kp_3d_all_cp[:, 138:, :]
    kp_3d_all_cp_smooth = np.concatenate((kp_3d_all_smooth, cp), axis=1)
    print(kp_3d_all_cp_smooth.shape)
    # print(used_finger)
...
if __name__ == '__main__':
    ...
    pitch_results = pitch_detect_crepe(proj_dir, True, 'wavs/cut_240102_1223(124).wav')
    # ic(pitch_results[352:360])
    # pitch_results = pitch_detect_pyin(proj_dir, 'wavs/scale.wav')
    pitch_with_positions = freq_position_violin.get_contact_position(pitch_results)
    ...
    new_positions = mapping(proj_dir, positions, visualize=True)
